...
export class AddRewardsComponent implements OnInit {
  ...
  Gifttype = [{label: "Cash", value: "cash"},{label: "Other", value: "other"}];
  ...
  onFormSubmit() {
    ...
    if (this.addRewardForm.valid) {
      ...
      // if (this.addRewardForm.value.giftType === "cash") {
        let form_data_submit ={...this.addRewardForm.value, "passengerId": this.passenger_id ,name :type} 
      // }else{
      //   let form_data_submit ={...this.addRewardForm.value, "driverId": this.passenger_id} 
      // }
      this.rewardService.AddDriversReward(form_data_submit)
        .subscribe(next => {
            ...
            if (next.status_code == 200) {
              ...
              this.router.navigate(["/reward/passenger-reward"]);
            } else {
