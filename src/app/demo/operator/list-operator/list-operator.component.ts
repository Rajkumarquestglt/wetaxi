...
export class ListOperatorComponent implements AfterViewInit, OnInit {
      ...
      ajax: (dataTablesParameters: any, callback) => {
        ...
        this.operatorService.listAllOperators(dataTablesParameters).subscribe(
          resp => {
            ...
            this.loading = false;
            this.operators = resp.data[0].promoterList;
  ...
  activeInactiveOperator(operator: any) {
    ...
    if (operator.isActive) {
      text = 'You want to inactive this Promoter ?';
    } else {
      text = 'You want to active this Promoter ?';
    }
        ...
        this.operatorService.activeInactiveOperator(data).subscribe(
          next => {
            if (next.status_code == 200) {
              this.rerender();
              if (operator.isActive) {
                Swal('Success', "promoter inactivated successfully.", 'success');
              } else {
                Swal('Success', "promoter activated successfully.", 'success');
              }
            } else {
              Swal('Error', "promoter status is not updated.", 'error');
            }
          },
          error => {
            Swal('Error', "promoter status is not updated.", 'error');
