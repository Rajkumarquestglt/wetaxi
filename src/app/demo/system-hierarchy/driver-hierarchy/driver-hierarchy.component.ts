  ...
  ngOnInit() {
    ...
    this.dtOptions = {
      ...
      stateLoadCallback: function(settings) {
        return JSON.parse(
          localStorage.getItem("DataTables_heirarchy_management")
        );
      },
      ajax: (dataTablesParameters: any, callback) => {
        ...
        this.refferalHierarchyService
          .ListAllDriverReferrals(dataTablesParameters)
          .subscribe(
            resp => {
              this.loading = false;
            ...
            for(var i = 0; i < inputs.length; i++) {
              ...
              for (let index = 0; index < this.drivers.length; index++) {
                if (
                  moment().format("D") ==
                    moment(this.drivers[index].dob).format("D") &&
                  moment().format("MMMM") ==
                    moment(this.drivers[index].dob).format("MMMM")
                ) {
...
const rows=[];
  ...
  exportExcelData() {
    ...
    this.refferalHierarchyService.ListAllDriverReferrals({}).subscribe(
      resp => {
    ...
    this.loading = false;
        ...
        this.excelService.exportAsExcelFile(this.exportExcel, 'Driver-hierarchy');
