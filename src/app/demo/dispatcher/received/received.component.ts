...
export class ReceivedComponent implements OnInit {
  ...
  receivedBooking: any;
  public monthltReceivedBooking;
  public weeklyReceivedBooking;
  exportExcelData = [];
  getPdfData: any;
  constructor(
  ...
  generatePdf() {
    ...
    this.loading = true;
    this.dispatcherService.getAllReceivedBookingsDispacterPDF().subscribe(
      (respone) => {
        this.getPdfData = respone.data.AllReceivedBookingsDispacter;
  ...
  exportDriverDataToPdf() {
    ...
    downloadPDF.save("received_booking.pdf");
  }

  exporExcel() {
    this.loading = true;

    this.dispatcherService.getAllReceivedBookingsDispacter1().subscribe(
      (resp) => {
        // console.log("log", resp.data.AllPromocode)

        resp.data.AllReceivedBookingsDispacter.map((element) => {
  ...
  ngOnInit() {
    ...
    this.dtOptions = {
      ...
      ajax: (dataTablesParameters: any, callback) => {
        ...
        this.dispatcherService
          .getAllReceivedBookingsDispacter(dataTablesParameters)
          .subscribe(
            (resp) => {
              this.loading = false;
              this.receivedBooking = resp.data.AllReceivedBookingsDispacter;
    ...
    this.getMonthlyReceivedBookingsCount();
    // this.dispatcherService
    // .getAllReceivedBookingsDispacter()
    // .subscribe(respone => {
    //   this.receivedBooking = respone.data.AllReceivedBookingsDispacter;

    //   console.log("receivedBooking", this.receivedBooking);
    // });
  }
  getMonthlyReceivedBookingsCount() {
    this.dispatcherService
      .getMonthlyReceivedBookingsCount()
      .subscribe((respone) => {
        this.monthltReceivedBooking = respone.data.MonthlyReceivedBookingsCount;
        this.zone.runOutsideAngular(() => {
          let lineChart = am4core.create(
            "monthlyDispatcherStaticsChart",
            am4charts.XYChart
          );
          let data = [
            { month: "01", count: 0 },
          ...
          for (
            let i = 0;
            i < respone.data.MonthlyReceivedBookingsCount.length;
            i++
          ) {
            data[
              Number(respone.data.MonthlyReceivedBookingsCount[i]._id.month) - 1
            ]["count"] = respone.data.MonthlyReceivedBookingsCount[i].count;
          }
          lineChart.data = data;
  ...
  getWeeklyReceivedBookingsCount() {
    ...
    this.zone.runOutsideAngular(() => {
      ...
      this.dispatcherService.getWeeklyReceivedBookingsCount().subscribe(
        (respone) => {
          // console.log("live monthly data",respone.data.monthlyLiveTripsCount[0]._id)
          this.weeklyReceivedBooking = respone.data.WeeklyReceivedBookingsCount;
          ...
          // }
          liveData = respone.data.WeeklyReceivedBookingsCount;
