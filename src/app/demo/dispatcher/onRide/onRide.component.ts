...
@Component({
  selector: "onRide",
  templateUrl: "./onRide.component.html",
})
export class OnRideComponent implements OnInit {
  ...
  filterValue: any = {};
  onRideBooking: any;
  monthltOnRideBooking: any;
  weeklyAcceptedOnRideBooking: any;
  exportExcelData = [];
  getPdfData: any;
  ...
  generatePdf() {
    ...
    this.loading = true;
    this.dispatcherService.getAllOnRideDispacterPDF().subscribe(
      (respone) => {
        this.getPdfData = respone.data.AllOnRideDispacter;
  ...
  exportDriverDataToPdf() {
    ...
    downloadPDF.save("OnRide_booking.pdf");
  }

  exporExcel() {
    this.loading = true;

    this.dispatcherService.getAllOnRideDispacter1().subscribe(
      (resp) => {
        // console.log("log", resp.data.AllPromocode)

        resp.data.AllOnRideDispacter.map((element) => {
  ...
  ngOnInit() {
    ...
    this.dtOptions = {
      ...
      ajax: (dataTablesParameters: any, callback) => {
        ...
        this.dispatcherService
          .getAllOnRideDispacter(dataTablesParameters)
          .subscribe(
            (resp) => {
              this.loading = false;
              this.onRideBooking = resp.data.AllOnRideDispacter;
    ...
    this.getMonthlyAcceptedOnRideDispacterCount();
    // this.dispatcherService.getAllOnRideDispacter().subscribe(respone => {
    //   this.onRideBooking = respone.data.AllOnRideDispacter;

    //   console.log("onRideBooking", this.onRideBooking);
    // });
  }
  getMonthlyAcceptedOnRideDispacterCount() {
    this.dispatcherService
      .getMonthlyAcceptedOnRideDispacterCount()
      .subscribe((respone) => {
        this.monthltOnRideBooking =
          respone.data.MonthlyAcceptedOnRideDispacterCount;
        console.log(
          "--------------->this.monthltOnRideBooking",
          this.monthltOnRideBooking
        );
        this.zone.runOutsideAngular(() => {
          ...
          for (
            let i = 0;
            i < respone.data.MonthlyAcceptedOnRideDispacterCount.length;
            i++
          ) {
            data[
              Number(
                respone.data.MonthlyAcceptedOnRideDispacterCount[i]._id.month
              ) - 1
            ]["count"] =
              respone.data.MonthlyAcceptedOnRideDispacterCount[i].count;
          }
          lineChart.data = data;
  ...
  getWeeklyAcceptedOnRideDispacterCount() {
    this.zone.runOutsideAngular(() => {
      ...
      this.dispatcherService.getWeeklyAcceptedOnRideDispacterCount().subscribe(
        (respone) => {
          // console.log("live monthly data",respone.data.monthlyLiveTripsCount[0]._id)
          this.weeklyAcceptedOnRideBooking =
            respone.data.WeeklyAcceptedOnRideDispacterCount;
          console.log(
            "this.weeklyAcceptedBooking",
            this.weeklyAcceptedOnRideBooking
          );
          ...
          // }
          liveData = respone.data.WeeklyAcceptedOnRideDispacterCount;
