      ...
      this.locationService.getTripsByDriverIdPdf(recycled).subscribe(
        resp => {
          ...
          resp.data.RideDetail.map(element => {
            this.exportDriverExcel.push({ 'Trip Id': element.rideId, 'Name': element.vehicleTypeData.type.en, 'Phone Number': element.driverData.onlyPhoneNumber,'totalDistance': element.totalDistance,'totalTime': element.totalTime,'totalFare': element.totalFare,'driverEarning': element.driverEarning })
          });
          this.loading = false;
    ...
    ngOnInit(){
      ...
      this.dtOptions = {
        ...
        columns: [
          ...
          { data: "totalFare",orderable: false, searchable: false},
          {data:null},
